...
import { PencilEdit, UserComment } from "@vectopus/atlas-icons-react";
import { styles } from "../assets/style";
import { useUpdateAlbum } from "../hooks/useAlbums";

export default function EditAlbum({ album, refetch }) {
  const {
    handleSubmit,
    control,
    register,
  ...
  const editAlbum = useUpdateAlbum({ albumId: album._id, refetch });
  ...
  const onSubmit = (data) => {
    ...
    editAlbum.mutate(formData);
  };

  return (
    <>
      <div onClick={onOpen} className="w-full flex justify-center">
        <PencilEdit size={22} weight="bold" />
      </div>
      <Modal
        size="5xl"
        isOpen={isOpen}
